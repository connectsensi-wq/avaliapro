...
export async function PUT(
  ...
  try {
    ...
    // 🚨 Remover do body os campos que pertencem a "retentions"
    const {
      inss_percentage,
      irpj_percentage,
      csll_percentage,
    ...
    const invoice = await db.invoice.update({
      ...
      data: {
        ...
        accounts_payable: body.accounts_payable
          ? {
              deleteMany: {}, // apaga todos os lançamentos anteriores
...
export async function DELETE(req: Request) {
  try {
    ...
    return NextResponse.json({ error: "Erro ao deletar invoice" }, { status: 500 });
  }
}