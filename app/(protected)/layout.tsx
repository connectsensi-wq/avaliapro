          ...
          <SidebarHeader className="border-b border-sidebar-border p-5 space-y-4">
            ...
            {companies.length > 0 && (
              <div className="space-y-1.5">
                <label className="text-[11px] font-semibold text-sidebar-primary uppercase tracking-wider flex items-center gap-1.5 opacity-90">
                    ...
                    {companies.map((c) => (
                      ...
                      >
                        <div className="flex flex-col">
                          <span className="font-medium text-sidebar-primary text-sm">{c.name}</span>
                        </div>
                      </SelectItem>
                    ))}
                  </SelectContent>
          ...
          <SidebarContent className="p-3 space-y-4">
            {navigationGroups.map((group) => (
              <SidebarGroup key={group.label} className="p-0">
                <SidebarGroupLabel className="text-[10px] font-bold text-sidebar-primary/70 uppercase tracking-widest px-3 py-1.5">
                  {group.label}
                </SidebarGroupLabel>
                <SidebarGroupContent>
                        ...
                        <SidebarMenuItem key={item.title}>
                          <SidebarMenuButton
                            ...
                            className={`group relative transition-all duration-200 rounded-xl px-3 py-2.5 flex items-center justify-between ${isActive
                              ? "bg-primary/15 text-sidebar-primary font-semibold border-l-4 border-primary shadow-sm"
                              : "text-muted-foreground hover:bg-secondary/70 hover:text-foreground"
                              }`}
                          >
                            <Link href={item.url} className="flex items-center gap-3 w-full">
                              <item.icon
                                className={`w-4 h-4 transition-transform duration-200 group-hover:scale-110 ${isActive ? "text-sidebar-primary" : "text-muted-foreground group-hover:text-sidebar-primary"
                                  }`}
                              />
                              <span className="text-sm font-medium">{item.title}</span>
                              {isActive && (
                                <ChevronRight className="w-3.5 h-3.5 ml-auto text-sidebar-primary opacity-80" />
                              )}
                            </Link>
                          </SidebarMenuButton>
                        </SidebarMenuItem>
            ...
            {selectedCompany && (
              <div className="bg-sidebar border border-border rounded-xl p-3 shadow-sm flex items-center justify-between">
                <div className="truncate space-y-0.5">
                  <p className="text-xs font-semibold text-sidebar-primary truncate">{selectedCompany.name}</p>
                ...
                <div className="h-2 w-2 rounded-full bg-sidebar-primary animate-pulse shrink-0" title="Empresa Ativa" />
              </div>
            )}
          ...
          <header className="h-16 bg-card border-b border-border px-4 sm:px-6 flex items-center justify-between shrink-0 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {selectedCompany && (
                  <span className="hidden md:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-secondary text-sidebar-primary border border-primary/20">
            ...
            <div className="flex items-center gap-3">
              <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-secondary/80 text-sidebar-primary border border-primary/30 shadow-sm">
                <ShieldCheck className="h-3.5 w-3.5 text-sidebar-primary" /> Sistema Ativo
              </span>
            </div>
          </header>
